...
-- * main functions
main = do [strConn] <- getArgs
          beam <- connectODBC strConn

          --withDatabase beam $           
          let (EmployeeDatabase employeesT departmentsT groupsT ordersT) = employeeDb
           ...
           {-------------------------
                ...
                employees = [ Employee 1 "James" "Smith" (GroupId "ItGroup1" "IT") 
                            , Employee 2 "Taylor" "Jones" (GroupId "ItGroup1" "IT")
                            , Employee 3 "Samantha" "Nolen" (GroupId "ItGroup2" "IT") 
                            , Employee 4 "Maurice" "Davies" (GroupId "OpsGroup1" "operations")
                            , Employee 5 "Bob" "Lee" (GroupId "OpsGroup2" "operations") 
                            , Employee 6 "Constantine" "Nevos" (GroupId "OpsGroup3" "operations")

                            , Employee 7 "Tom" "Jones" (GroupId "Alpha" "sales") 
                            , Employee 8 "Toby" "Roberts" (GroupId "Alpha" "sales")
                            , Employee 9 "Katy" "Barry" (GroupId "Beta" "sales") 
                            , Employee 10 "Amy" "Zely" (GroupId "Beta" "sales") 
                            , Employee 11 "Pierre" "Berger" (GroupId "Gamma" "sales")
                            , Employee 12 "Blaise" "Solle" (GroupId "Gamma" "sales")  ]
            ...
            runInsert $ insert departmentsT $ insertValues departments
            runInsert $ insert groupsT $ insertValues groups
...
-------------}
          ...
          q1 <- runFirebird beam $ runSelectReturningList $ select $ all_ departmentsT
